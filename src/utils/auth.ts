...
export function setToken(data: DataInfo<Date>) {
  ...
  function setSessionKey(
    ...
    useUserStoreHook().SET_ROLECODE(roleCode);
    useUserStoreHook().SET_ACCOUNTID(accountId);
  ...
  if (data.username && data.roles && data.roleId) {
    const { username, userType, roles, roleId, roleCode, userId, accountId } =
      data;
    setSessionKey(
      username,
      userType,
    ...
    const roleId =
      storageSession().getItem<DataInfo<number>>(sessionKey)?.roleId ?? 0;
    const userId =
